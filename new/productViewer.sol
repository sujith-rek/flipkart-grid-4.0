...
contract ProductDetailsViewer is ProductManufacture{
    ...
    function viewWarrantyStatus(
        uint _tokenId
            ...
            if((_product[_tokenId].LatestPurchaseDate + _product[_tokenId].WarrantyPeriod) > block.timestamp){
                return ("Active",
                       (_product[_tokenId].LatestPurchaseDate + _product[_tokenId].WarrantyPeriod - block.timestamp)/86400 );
            } else {
                return ("Expired", 0);
            }
    }
